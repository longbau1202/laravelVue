...
on-app-prod: ## On app in production mode
	@echo "List iDaas TAG image exits on your computer:"
...
build-prod: ## Docker build iDaas production image
	@printf "To run in non-interactive mode please run command bellow:\n\t";\
	printf "${TITLE_COLOR}TAG=iDaas_tag AWS_ACCOUNT_ID=your_aws_account make build-prod-fargate${NO_COLOR}\n";\
	\
	if [ -z "$$TAG" ]; then\
		read -p "iDaas docker image TAG [`date +'%Y.%m.%d'`] TAG=" TAG;\
	fi;\
	\
	...
	TAG=$${TAG:=`date +'%Y.%m.%d'`};\
	printf "${TITLE_COLOR}Building iDaas with TAG='$$TAG' and push to aws account AWS_ACCOUNT_ID='$$AWS_ACCOUNT_ID'${NO_COLOR} ...\n";\
	\
	...
	\
	docker tag idaas-public:$${TAG} $${AWS_ACCOUNT_ID}.dkr.ecr.ap-northeast-1.amazonaws.com/idaas-public:$${TAG};\
	docker tag idaas-app:$${TAG} $${AWS_ACCOUNT_ID}.dkr.ecr.ap-northeast-1.amazonaws.com/idaas-app:$${TAG};\
...
docker-push: ## Docker push faragte image. Make sure you run docker-login first
	@if [ -z "$$AWS_ACCOUNT_ID" -o -z "$$TAG" ]; then\
		printf "To push iDaas fargate image please run:\n";\
		printf "${TITLE_COLOR}AWS_ACCOUNT_ID=x TAG=y make docker-push${NO_COLOR}\n";\
	else\
		docker push $${AWS_ACCOUNT_ID}.dkr.ecr.ap-northeast-1.amazonaws.com/idaas-public:$${TAG};\
		docker push $${AWS_ACCOUNT_ID}.dkr.ecr.ap-northeast-1.amazonaws.com/idaas-app:$${TAG};\
	fi
...
test: on-app## Run phpunit test app integration with database
	docker exec -t idaas_app tests/phpunit --no-coverage --testdox -c tests/phpunit.xml

test-library: on-app ## Run phpunit test library only - testdox
	docker exec -t idaas_app tests/phpunit --no-coverage --testdox -c tests/phpunit.lib.xml

test-coverage: on-app ## Measure test coverage
	docker exec -t -e XDEBUG_MODE=coverage idaas_app tests/phpunit -d memory_limit=-1 -c tests/phpunit.xml

clean-test-coverage: ## Clean test coverage results
	rm -rf tests/coverage

ssh-app: ## SSH into iDaas app container
	docker exec -it laravelvue_app sh

ssh-app-as-root: ## SSH into iDaas app container as root user
	docker exec -it --user 0 laravelvue_app sh

show-log-app: ## Show log iDaas app container
	docker logs -f laravelvue_app

show-log-app-nginx: ## Show log iDaas nginx container
	docker logs -f laravelvue_public

show-log-db: ## Show log mysql server container
	docker logs -f laravelvue_db
...
# 		docker-compose exec idaas_db mysql -pcdmllove $$dbname -e "update crm_member set member_used_2auth=0 where member_id=1;";\
# 	done
#
